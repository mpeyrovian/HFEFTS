DELIMITER //
CREATE TRIGGER tr_schedule_date_range_update
...
DELIMITER //
CREATE TRIGGER `tr_check_schedule_conflicts_update`
    BEFORE update on  `schedule`
    FOR EACH ROW
BEGIN
    IF EXISTS (
...
DELIMITER //
create TRIGGER tr_schedule_time_gap_exists_update
    BEFORE update on  schedule
    FOR EACH ROW
BEGIN
    IF EXISTS (
...
DELIMITER //
CREATE TRIGGER `send_schedule_email_update`
...
DELIMITER //
CREATE TRIGGER `covid_schedule_check_update` BEFORE update on  `schedule`
    FOR EACH ROW
BEGIN
...
DELIMITER //
CREATE TRIGGER `employee_infected_cancel_schedule_update`
    AFTER update on  `infection`
    FOR EACH ROW
BEGIN
